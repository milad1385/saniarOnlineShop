    ...
    <Link to={`/category-products/${category.link}`}>
      <div className="bg-white flex items-center justify-between md:hover:-translate-y-[6px] p-3 md:p-4 rounded-md shadow-md transition-all">
        ...
        <div className="flex items-center flex-col gap-y-4">
          <Link
            to={`/category-products/${category.link}`}
            className="font-DanaMedium text-sm md:text-base"
          >
            {category.title}
          </Link>
          <Link
            to={`/category-products/${category.link}`}
            className="flex items-center justify-between gap-x-2 cursor-pointer text-xs md:text-sm bg-gray-200 shadow px-2.5 py-1.5 border border-gray-300 hover:bg-white transition-colors rounded-md"
          >
            مشاهده
